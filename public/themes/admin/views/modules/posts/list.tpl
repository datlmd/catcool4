		...
		<div class="row">
			...
			<div class="col-sm-7 col-12 mb-2 mb-sm-0 text-end">
				...
				<ul class="dropdown-menu" aria-labelledby="btn_group_drop_setting">
					<li><a class="dropdown-item" href="{site_url('manage/post_categories')}">{lang('CategoryAdmin.heading_title')}</a></li>
					<li><a class="dropdown-item" href="{site_url('manage/translations')}?module_id={lang('PostAdmin.translate_frontend_id')}">{lang("Admin.text_translate")}</a></li>
					<li><a class="dropdown-item" href="{site_url('manage/translations')}?module_id={lang('PostAdmin.translate_admin_id')}">{lang("Admin.text_translate_admin")}</a></li>
				</ul>
			</div>
		</div>
								...
								<table class="table table-striped table-hover table-bordered second">
									...
									{foreach $list as $item}
										<tr id="item_id_{$item.post_id}">
											<td class="text-center">{form_checkbox('manage_ids[]', $item.post_id)}</td>
											...
											<td>
												...
												<div class="form-check form-switch form-check-inline">
													{form_checkbox("is_homepage_`$item.post_id`", ($item.is_homepage eq STATUS_ON) ? true : false, ($item.is_homepage eq STATUS_ON) ? true : false, ['id' => 'is_homepage_'|cat:$item.post_id, 'data-id' => $item.post_id, 'data-is-homepage' => $item.is_homepage, 'class' => 'change_homepage form-check-input'])}
													<label class="form-check-label me-3" for="is_homepage_{$item.post_id}">{lang('NewsAdmin.text_is_homepage')}</label>
												</div>
												<div class="form-check form-switch form-check-inline">
													{form_checkbox("is_hot_`$item.post_id`", ($item.is_hot eq STATUS_ON) ? true : false, ($item.is_hot eq STATUS_ON) ? true : false, ['id' => 'is_hot_'|cat:$item.post_id, 'data-id' => $item.post_id, 'data-is-hot' => $item.is_hot, 'class' => 'change_hot form-check-input'])}
													<label class="form-check-label" for="is_hot_{$item.post_id}">{lang('NewsAdmin.text_is_hot')}</label>
												</div>

											</td>
											<td>
												<div class="switch-button switch-button-xs catcool-center">
													{form_checkbox("published_`$item.post_id`", ($item.published eq STATUS_ON) ? true : false, ($item.published eq STATUS_ON) ? true : false, ['id' => 'published_'|cat:$item.post_id, 'data-id' => $item.post_id, 'data-published' => $item.published, 'class' => 'change_publish'])}
													<span><label for="published_{$item.post_id}"></label></span>
												</div>
											</td>
											<td class="text-center">
												<div class="btn-group ms-auto">
													...
													<a href="{site_url($manage_url)}/edit/{$item.post_id}" class="btn btn-sm btn-light" data-bs-toggle="tooltip" title="{lang('Admin.button_edit')}"><i class="fas fa-edit"></i></a>
													<button type="button" data-id="{$item.post_id}" data-is-trash="1" class="btn btn-sm btn-light text-danger btn_delete_single" data-bs-toggle="tooltip" title="{lang('Admin.text_move_to_trash')}"><i class="fas fa-trash-alt"></i></button>
												</div>
											</td>
										</tr>
									{/foreach}
		...
		<div class="modal-dialog modal-lg modal-dialog-centered">
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
						{form_open('manage/posts/robot', ['id' => 'robot_news_form'])}
	...
	<script>
		...
		function robotNewsScan() {
			...
			if ($('#robot_news input[name="url"]').val() != "") {
				window.location.href = 'manage/posts/add?url=' + $('#robot_news input[name="url"]').val();
