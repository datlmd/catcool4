	...
	<div class="container-fluid dashboard-content">
		<div class="row">
			...
			<div class="col-sm-5 col-12 mb-2 mb-sm-0 text-end">
				...
				<a href="{site_url($manage_url)}/add" class="btn btn-sm btn-primary btn-space" title="{lang('PageAdmin.text_add')}"><i class="fas fa-plus me-1"></i>{lang('PageAdmin.text_add')}</a>
				...
				{include file=get_theme_path('views/inc/button_translate.tpl') translate_frontend=lang('PageAdmin.translate_frontend_id') translate_admin=lang('PageAdmin.translate_admin_id')}
			</div>
		</div>
						...
						<div class="card-body">
							<div class="row">
								...
								<div class="col-xl-4 col-lg-4 col-md-4 col-sm-6 col-12 mb-2">
									...
									{form_input('page_id]', set_value('page_id', $filter.page_id), ['class' => 'form-control form-control-sm', 'placeholder' => lang('Admin.filter_id')])}
		...
		<div class="row">
			<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
				<div class="card">
					<h5 class="card-header"><i class="fas fa-list me-2"></i>{lang('PageAdmin.text_list')}</h5>
					<div class="card-body">
						{if !empty($list)}
								...
								<table class="table table-striped table-hover table-bordered second">
									<thead>
										<tr class="text-center">
											<th width="50">
												<a href="{site_url($manage_url)}?sort=page_id&order={$order}{$url}" class="text-dark">
									...
									{foreach $list as $item}
										<tr>
											<td class="text-center">{anchor("$manage_url/edit/`$item.page_id`", $item.page_id, 'class="text-primary"')}</td>
											<td>
											...
											<td>
												<div class="switch-button switch-button-xs catcool-center">
													...
													<span><label for="published_{$item.page_id}"></label></span>
												</div>
											</td>
											<td class="text-center">
												<div class="btn-group ms-auto">
													<a href="{site_url($manage_url)}/edit/{$item.page_id}" class="btn btn-sm btn-light" title="{lang('Admin.button_edit')}"><i class="fas fa-edit"></i></a>
													<button type="button" data-id="{$item.page_id}" class="btn btn-sm btn-light text-danger btn_delete_single" title="{lang('Admin.button_delete')}"><i class="fas fa-trash-alt"></i></button>
												</div>
											</td>
											<td class="text-center">{form_checkbox('manage_ids[]', $item.page_id)}</td>
										</tr>
									{/foreach}
									</tbody>
								</table>
							</div>
							{include file=get_theme_path('views/inc/paging.tpl') pager_name='pages'}
						{else}
