{strip}
		...
		<div class="row">
			...
			<div class="col-sm-5 col-12 mb-2 mb-sm-0 text-end">
				...
				<ul class="dropdown-menu" aria-labelledby="btn_group_drop_setting">
					...
					<li><a class="dropdown-item" href="{site_url('manage/translations')}?module_id={lang('NewsAdmin.translate_frontend_id')}">{lang("Admin.text_translate")}</a></li>
					<li><a class="dropdown-item" href="{site_url('manage/translations')}?module_id={lang('NewsAdmin.translate_admin_id')}">{lang("Admin.text_translate_admin")}</a></li>
				</ul>
			</div>
		</div>
									...
									{foreach $list as $item}
										<tr id="item_id_{$item.news_id}">
											...
											<td>
												...
												<a href="{site_url($manage_url)}/restore/{$item.news_id}" class="btn btn-sm btn-light text-dark" title="{lang('Admin.button_restore')}"><i class="fas fa-reply"></i>&nbsp;{lang('Admin.button_restore')}</a>
												<button type="button" data-id="{$item.news_id}" class="btn btn-sm btn-danger ms-2 btn_delete_single" title="{lang('Admin.button_delete_permanently')}"><i class="fas fa-trash-alt"></i>&nbsp;{lang('Admin.button_delete_permanently')}</button>
	...
	<!-- Modal add -->
	<div class="modal fade" id="empty_trash" tabindex="-1" role="dialog" aria-labelledby="robotNewsModalLabel" aria-hidden="true">
