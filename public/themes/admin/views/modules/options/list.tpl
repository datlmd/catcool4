			...
			<div class="col-12">
				...
				<div class="row">
					...
					<div class="col-sm-5 col-12 mb-2 mb-sm-0 text-end">
						...
						{include file=get_theme_path('views/inc/button_translate.tpl') translate_frontend=lang('OptionAdmin.translate_frontend_id') translate_admin=lang('OptionAdmin.translate_admin_id')}
					</div>
				</div>

				<div class="card">
					<h5 class="card-header"><i class="fas fa-list me-2"></i>{lang('OptionAdmin.text_list')}</h5>
					<div class="card-body">
						{if !empty($list)}
										...
										<tr id="item_id_{$item.option_id}">
											...
											<td>
												<div class="switch-button switch-button-xs catcool-center">
													{form_checkbox("published_`$item.option_id`", ($item.published eq STATUS_ON) ? true : false, ($item.published eq STATUS_ON) ? true : false, ['id' => 'published_'|cat:$item.option_id, 'data-id' => $item.option_id, 'data-published' => $item.published, 'class' => 'change_publish'])}
													<span><label for="published_{$item.option_id}"></label></span>
												</div>
											</td>
