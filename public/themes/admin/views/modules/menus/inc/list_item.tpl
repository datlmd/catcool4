...
<li class="dd-item" data-id="{$menu.menu_id}">
	<div class="dd-handle">
		...
		<div class="dd-nodrag btn-group ms-auto">
			...
			<a href="{site_url($manage_url)}/edit/{$menu.menu_id}{http_get_query()}" class="btn btn-sm btn-outline-light" title="{lang('Admin.button_edit')}"><i class="fas fa-edit"></i></a>
			<button type="button" data-id="{$menu.menu_id}" class="btn btn-sm btn-outline-light text-danger btn_delete_single" title="{lang('Admin.button_delete')}"><i class="fas fa-trash-alt"></i></button>
		</div>
	</div>
	{if !empty($menu.subs)}
		<ol class="dd-list">
