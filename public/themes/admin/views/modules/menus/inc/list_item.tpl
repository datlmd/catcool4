<li class="dd-item" data-id="{$menu.menu_id}">
	<div class="dd-handle">
		...
		<div class="dd-nodrag btn-group ms-auto">
			...
			<a href="{site_url($manage_url)}/edit/{$menu.menu_id}{http_get_query()}" class="btn btn-sm btn-outline-light" {if count($list) > 1}data-bs-toggle="tooltip" data-placement="top" title="" data-bs-original-title="{lang('button_edit')}"{/if}><i class="fas fa-edit"></i></a>
			<button type="button" data-id="{$menu.menu_id}" class="btn btn-sm btn-outline-light text-danger btn_delete_single" {if count($list) > 1}data-bs-toggle="tooltip" data-placement="top" title="" data-bs-original-title="{lang('button_delete')}"{/if}><i class="fas fa-trash-alt"></i></button>
		</div>
	</div>
	{if !empty($menu.subs)}
		<ol class="dd-list">
