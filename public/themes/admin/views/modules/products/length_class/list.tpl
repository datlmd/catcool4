							...
							<table class="table table-striped table-hover table-bordered second">
								...
								{foreach $list as $item}
									<tr>
										...
										<td>{anchor("$manage_url/edit/`$item.length_class_id`", $item.detail.name, 'class="text-primary"')}</td>
										<td>{$item.detail.unit}</td>
										<td>{$item.value}</td>
										<td class="text-center">
											<div class="btn-group ms-auto">
												<a href="{$manage_url}/edit/{$item.length_class_id}" class="btn btn-sm btn-outline-light" {if count($list) > 1}data-bs-toggle="tooltip" data-placement="top" title="" data-original-title="{lang('button_edit')}"{/if}><i class="fas fa-edit"></i></a>
												<button type="button" data-id="{$item.length_class_id}" class="btn btn-sm btn-outline-light text-danger btn_delete_single" {if count($list) > 1}data-bs-toggle="tooltip" data-placement="top" title="" data-original-title="{lang('button_delete')}"{/if}><i class="fas fa-trash-alt"></i></button>
											</div>
										</td>
