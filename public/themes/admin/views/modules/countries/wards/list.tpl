				...
				{form_open(uri_string(), ['id' => 'filter_validationform', 'method' => 'get'])}
					...
					<div class="card-body">
						<div class="row">
							...
							<div class="col-xl-4 col-lg-4 col-md-4 col-sm-6 col-12 mb-2">
								{lang('text_district')}
								{form_dropdown('filter[district_id]', $district_list, set_value('filter[district_id]', $this->input->get('filter[district_id]')), ['class' => 'form-control'])}
							</div>
							...
							<table class="table table-striped table-hover table-bordered second">
								...
								{foreach $list as $item}
									<tr>
										...
										<td>{$item.description} {anchor("$manage_url/edit/`$item.ward_id`", htmlspecialchars($item.name, ENT_QUOTES,'UTF-8'), 'class="text-primary"')}</td>
										...
										<td class="text-center">
											<div class="btn-group ms-auto">
												<a href="{$manage_url}/edit/{$item.ward_id}" class="btn btn-sm btn-outline-light" {if count($list) > 1}data-bs-toggle="tooltip" data-placement="top" title="" data-original-title="{lang('button_edit')}"{/if}><i class="fas fa-edit"></i></a>
												<button type="button" data-id="{$item.ward_id}" class="btn btn-sm btn-outline-light text-danger btn_delete_single" {if count($list) > 1}data-bs-toggle="tooltip" data-placement="top" title="" data-original-title="{lang('button_delete')}"{/if}><i class="fas fa-trash-alt"></i></button>
											</div>
										</td>
