				...
				{form_open(uri_string(), ['id' => 'filter_validationform', 'method' => 'get'])}
					...
					<div class="card-body">
						<div class="row">
							...
							<div class="col-xl-4 col-lg-4 col-md-4 col-sm-6 col-12 mb-2">
								{lang('text_country')}
								{form_dropdown('filter[country_id]', $country_list, set_value('filter[country_id]', $this->input->get('filter[country_id]')), ['class' => 'form-control'])}
							</div>
									...
									<tr>
										...
										<td class="text-center">
											<div class="btn-group ms-auto">
												<a href="{$manage_url}/edit/{$item.province_id}" class="btn btn-sm btn-outline-light" {if count($list) > 1}data-bs-toggle="tooltip" data-placement="top" title="" data-original-title="{lang('button_edit')}"{/if}><i class="fas fa-edit"></i></a>
												<button type="button" data-id="{$item.province_id}" class="btn btn-sm btn-outline-light text-danger btn_delete_single" {if count($list) > 1}data-bs-toggle="tooltip" data-placement="top" title="" data-original-title="{lang('button_delete')}"{/if}><i class="fas fa-trash-alt"></i></button>
											</div>
										</td>
