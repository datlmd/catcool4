			...
			<ul class="navbar-nav ms-auto navbar-right-top">
				...
				<li class="nav-item dropdown nav-user">
					...
					<div class="dropdown-menu dropdown-menu-end nav-user-dropdown" aria-labelledby="navbar_dropdown_user_info">
						...
						<a class="dropdown-item" href="{site_url('manage/users/edit/'|cat:session('user_info.user_id'))}"><i class="fas fa-user-circle me-2"></i>{lang('Admin.text_profile')}</a>
						{if !empty(config_item('enable_dark_mode'))}
							<a class="dropdown-item" href="{site_url('manage/configs/config')}?k=dark_mode&v=0"><i class="far fa-lightbulb me-2"></i>{lang('ConfigAdmin.text_light_mode')}</a>
						{else}
							<a class="dropdown-item" href="{site_url('manage/configs/config')}?k=dark_mode&v=1"><i class="fas fa-moon me-2"></i>{lang('ConfigAdmin.text_dark_mode')}</a>
						{/if}
						{if !empty(config_item('enable_scroll_menu_admin'))}
							<a class="dropdown-item" href="{site_url('manage/configs/config')}?k=collapse_menu&v=0"><i class="fas fa-expand me-2"></i>{lang('ConfigAdmin.text_collapse_menu')}</a>
						{else}
							<a class="dropdown-item" href="{site_url('manage/configs/config')}?k=collapse_menu&v=1"><i class="fas fa-expand me-2"></i>{lang('ConfigAdmin.text_expand_menu')}</a>
						{/if}
						{if !empty(config_item('enable_icon_menu_admin'))}
							<a class="dropdown-item" href="{site_url('manage/configs/config')}?k=style_menu&v=0"><i class="fas fa-th-list me-2"></i>{lang('ConfigAdmin.text_list_menu')}</a>
						{else}
							<a class="dropdown-item" href="{site_url('manage/configs/config')}?k=style_menu&v=1"><i class="fas fa-th-large me-2"></i>{lang('ConfigAdmin.text_icon_menu')}</a>
						{/if}
						<a class="dropdown-item" href="{site_url('manage/users/logout')}"><i class="fas fa-sign-out-alt me-2"></i>{lang('Admin.text_logout')}</a>
					</div>
				</li>
					...
					<li class="nav-item dropdown nav-more">
						...
						<div class="dropdown-menu dropdown-menu-end nav-user-dropdown navbar-dropdown-menu-top" aria-labelledby="navbar_dropdown_menu_all">
							...
							{foreach $menu_admin as $key => $item}
								<a class="dropdown-item {if $item.is_active}active{/if}" href="{$item.slug}" {if !empty($item.nav_key)}id={$item.nav_key}{/if} {$item.attributes}>
									{if !empty($item.icon)}<i class="{$item.icon} me-2"></i>{/if}{$item.name}
								</a>
								{if !empty($item.subs)}
									{foreach $item.subs as $sub}
										<a class="dropdown-item py-2 {if $sub.is_active}active{/if}" href="{$sub.slug}" {$sub.attributes}><i class="fas fa-angle-double-right ms-3 me-2"></i>{$sub.name}</a>
									{/foreach}
								{/if}
							{/foreach}
						</div>
