						...
						<a href="{site_url($news.detail_url)}">
							<article class="thumb-info thumb-info-no-borders thumb-info-bottom-info thumb-info-bottom-info-dark thumb-info-bottom-info-show-more thumb-info-no-zoom border-radius-0">
								<div class="thumb-info-wrapper thumb-info-wrapper-opacity-6">
									<img src="{if !empty($news.images.thumb)}{image_thumb_url($news.images.thumb)}{else}{image_thumb_url($news.images.robot)}{/if}" class="img-fluid" width="100%" alt="{htmlentities($news.name)}">
				...
				{foreach $slide_list as $news}
					...
					<article class="thumb-info thumb-info-side-image thumb-info-no-zoom bg-transparent border-radius-0 pb-1 mb-2">
						<div class="row pb-1">
							<div class="col-sm-5">
								<a href="{site_url($news.detail_url)}">
									<img src="{if !empty($news.images.thumb)}{image_thumb_url($news.images.thumb)}{else}{image_thumb_url($news.images.robot)}{/if}" class="img-fluid border-radius-0" width="100%" alt="{htmlentities($news.name)}">
								</a>
							</div>
