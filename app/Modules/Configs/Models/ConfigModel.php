...
class ConfigModel extends MyModel
{
    ...
    public const CONFIG_CACHE_NAME = PREFIX_CACHE_NAME_MYSQL.'config_list';
    public const CONFIG_CACHE_EXPIRE = YEAR;
        ...
        try {
            ...
            if (!empty($settings)) {
                foreach ($settings as $setting) {
                    ...
                    if ((is_numeric($config_value) && $setting['config_key'] != 'store_phone')
                        || is_bool($config_value)
                        || in_array($config_value, ['true', 'false', 'TRUE', 'FALSE'])
