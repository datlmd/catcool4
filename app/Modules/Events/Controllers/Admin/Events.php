...
class Events extends AdminController
{
    ...
    public function __construct()
    {
        ...
        $this->breadcrumb->add(lang('EventAdmin.heading_title'), site_url(self::MANAGE_URL));
    }

    public function index()
    {
        add_meta(['title' => lang('EventAdmin.heading_title')], $this->themes);

        $limit = $this->request->getGet('limit');
        $sort = $this->request->getGet('sort');
        $order = $this->request->getGet('order');
        $filter_keys = ['event_id', 'code', 'priority', 'limit'];
    ...
    public function save()
    {
        ...
        $data_event = [
            ...
            'published' => $this->request->getPost('published') ? STATUS_ON : STATUS_OFF,
            'priority' => $this->request->getPost('priority'),
        ];

        if (empty($event_id)) {
            //Them moi
            $event_id = $this->model->insert($data_event);
            if (empty($event_id)) {
                $json['error'] = lang('Admin.error');
            }
        } else {
            //cap nhat
            $data_event['event_id'] = $event_id;
            if (!$this->model->save($data_event)) {
                $json['error'] = lang('Admin.error');
            }
        }
        ...
        if (!empty($this->request->getPost('event_id'))) {
    ...
    {
        $this->validator->setRule('code', lang('EventAdmin.text_code'), 'required|min_length[3]|max_length[63]');
        $this->validator->setRule('action', lang('EventAdmin.text_action'), 'required|min_length[3]');
        $this->validator->setRule('priority', lang('EventAdmin.text_priority'), 'required|is_natural');
