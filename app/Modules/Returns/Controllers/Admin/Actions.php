...
class Actions extends AdminController
{
    ...
    public function __construct()
    {
        ...
        $this->model_lang = new ActionLangModel();
        ...
        $this->breadcrumb->add(lang('ReturnActionAdmin.heading_title'), site_url(self::MANAGE_URL));
    }

    public function index()
    {
        add_meta(['title' => lang('ReturnActionAdmin.heading_title')], $this->themes);

        $limit = $this->request->getGet('limit');
        $sort = $this->request->getGet('sort');
        $order = $this->request->getGet('order');
        $filter_keys = ['return_action_id', 'name', 'limit'];
        ...
        $data = [
            ...
            'pager' => $list->pager,
            'sort' => empty($sort) ? 'return_action_id' : $sort,
    ...
    public function add()
    {
        if (!empty($this->request->getPost())) {
            ...
            foreach (list_language_admin() as $language) {
                ...
                $add_data_lang[$language['id']]['return_action_id'] = $id;
    ...
    public function edit($id = null)
    {
        ...
        if (!empty($this->request->getPost()) && $id == $this->request->getPost('return_action_id')) {
            if (!$this->_validateForm()) {
                set_alert([ALERT_ERROR => $this->errors]);
            ...
            foreach (list_language_admin() as $language) {
                ...
                $edit_data_lang[$language['id']]['return_action_id'] = $id;

                if (!empty($this->model_lang->where(['return_action_id' => $id, 'language_id' => $language['id']])->find())) {
    ...
    private function _getForm($id = null)
    {
        ...
        if (!empty($id) && is_numeric($id)) {
            ...
            $data_languages = $this->model_lang->where('return_action_id', $id)->findAll();
    ...
    public function delete($id = null)
    {
        ...
        if (!empty($this->request->getPost('is_delete')) && !empty($this->request->getPost('ids'))) {
            ...
            $list_delete = $this->model->getReturnActionsByIds($ids, $this->language_id);
        ...
        $list_delete = $this->model->getReturnActionsByIds($delete_ids, $this->language_id);
        if (empty($list_delete)) {
            json_output(['token' => $token, 'status' => 'ng', 'msg' => lang('Admin.error_empty')]);
        }
