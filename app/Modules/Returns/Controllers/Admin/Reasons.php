...
class Reasons extends AdminController
{
    ...
    public function __construct()
    {
        ...
        $this->model = new ReasonModel();
        $this->model_lang = new ReasonLangModel();
	...
	public function index()
	{
        ...
        $filter_keys = ['return_reason_id', 'name', 'limit'];
	    ...
	    $data = [
            ...
            'pager'         => $list->pager,
            'sort'          => empty($sort) ? 'return_reason_id' : $sort,
    ...
    public function add()
    {
        if (!empty($this->request->getPost())) {
            ...
            foreach (list_language_admin() as $language) {
                ...
                $add_data_lang[$language['id']]['return_reason_id'] = $id;
    ...
    public function edit($id = null)
    {
        ...
        if (!empty($this->request->getPost()) && $id == $this->request->getPost('return_reason_id')) {
            if (!$this->_validateForm()) {
                set_alert([ALERT_ERROR => $this->errors]);
                return redirect()->back()->withInput();
            }
    ...
    private function _getForm($id = null)
    {
        ...
        if (!empty($id) && is_numeric($id)) {
            ...
            $data_languages = $this->model_lang->where('return_reason_id', $id)->findAll();
    ...
    public function delete($id = null)
    {
        ...
        if (!empty($this->request->getPost('is_delete')) && !empty($this->request->getPost('ids'))) {
            ...
            $list_delete = $this->model->getReturnReasonsByIds($ids, $this->language_id);
        ...
        $list_delete = $this->model->getReturnReasonsByIds($delete_ids, $this->language_id);
        if (empty($list_delete)) {
            json_output(['token' => $token, 'status' => 'ng', 'msg' => lang('Admin.error_empty')]);
        }
        ...
        json_output(['token' => $token, 'data' => $this->themes::view('reasons/delete', $data)]);
    }
