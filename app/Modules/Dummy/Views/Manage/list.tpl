					...
					<div class="card-body">
						<div class="row">
							<div class="col-xl-4 col-lg-4 col-md-4 col-sm-6 col-12 mb-2">
								{lang('filter_name')}
								{form_input('filter[name]', $filter.name, ['class' => 'form-control form-control-sm', 'placeholder' => lang('filter_name')])}
							</div>
							<div class="col-xl-4 col-lg-4 col-md-4 col-sm-6 col-12 mb-2">
								{lang('filter_id')}
								{form_input('filter[id]', $filter.name, ['class' => 'form-control form-control-sm', 'placeholder' => lang('filter_id')])}
							</div>
							<div class="col-xl-4 col-lg-4 col-md-4 col-sm-6 col-12 mb-2">
								{lang('text_limit')}
								{form_dropdown('filter_limit', get_list_limit(), $filter.filter_limit, ['class' => 'form-control form-control-sm'])}
							</div>
						</div>
					</div>
                {form_close()}
							...
							<table class="table table-striped table-hover table-bordered second">
								...
								{foreach $list as $item}
									<tr>
										...
										<td>{anchor("$manage_url/edit/`$item.dummy_id`", $item.detail.name, 'class="text-primary"')}</td>
										...
										<td class="text-center">
											<div class="btn-group ml-auto">
												...
												<button type="button" data-id="{$item.dummy_id}" class="btn btn-sm btn-outline-light text-danger btn_delete_single" {if count($list) > 1}data-toggle="tooltip" data-placement="top" title="" data-original-title="{lang('button_delete')}"{/if}><i class="fas fa-trash-alt"></i></button>
											</div>
										</td>
