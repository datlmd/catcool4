...
if(!function_exists('create_input_token'))
{
    ...
    {
        if (empty($csrf) || !isset($csrf[config('Config')->csrf_name_key]) || !isset($csrf[config('Config')->csrf_name_value])) {
            return NULL;
        }

        return '
            <input type="hidden" name="' . config('Config')->csrf_name_key . '" value="' . $csrf[config('Config')->csrf_name_key] . '">
            <input type="hidden" name="' . config('Config')->csrf_name_value . '" value="' . $csrf[config('Config')->csrf_name_value] . '">
        ';
    }
}
...
if(!function_exists('script_global'))
{
    function script_global()
    {
        return '
            ...
            var global_username = "' . session('username') . '";
        ';
    }
}
...
if(!function_exists('check_captcha'))
{
    function check_captcha($captcha = '')
    {
        ...
        if (md5(strtolower($captcha) . config('Config')->catcool_hash) != $_SESSION['captcha']) {
            return false;
        }
